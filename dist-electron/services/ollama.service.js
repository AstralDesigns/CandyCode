 ...
 */
function convertToOllamaTools(geminiTools) {
    const tools = [];
...
class OllamaService {
    ...
    ollama = new ollama_1.Ollama({
        ...
        fetch: async (input, init) => {
            const response = await (0, node_fetch_1.default)(input, {
                ...init,
                timeout: 300000
            });
            ...
            if (response.body && typeof response.body.getReader !== 'function') {
                ...
                body.getReader = function () {
                    ...
                    return {
                        ...
                        releaseLock: () => { },
                        cancel: async () => {
                            if (typeof iterable.destroy === 'function')
                                iterable.destroy();
                        }
                    };
                }.bind(body);
                ...
                if (cloned.body && typeof cloned.body.getReader !== 'function') {
                    ...
                    cBody.getReader = function () {
                        return {
                            ...
                            releaseLock: () => { },
                            cancel: async () => { if (typeof cBody.destroy === 'function')
                                cBody.destroy(); }
                        };
                    }.bind(cBody);
                }
    ...
    async searchModels(query) {
        try {
            ...
            const filtered = popularModels.filter(m => m.name.toLowerCase().includes(query.toLowerCase()) ||
    ...
    async chatStream(prompt, options = {}, onChunk) {
        ...
        try {
            const tools = convertToOllamaTools(tool_definitions_1.TOOL_DEFINITIONS);
     ...
     */
    async checkForManualToolCalls(text) {
            ...
            if (match) {
                ...
                try {
                    ...
                    if (!this._processedLines)
                        this._processedLines = new Set();
                    if (this._processedLines.has(line))
                        continue;
