...
const isCleanInstall = process.argv.includes('--clean');
...
function setupNvmWindows(requiredVersion) {
  try {
    if (!isNvmWindowsInstalled()) {
      ...
      console.log('\nnvm-windows installed. Please restart your terminal and run the install script again.');
      return false;
    } else {
...
/**
 * Get the release directory for packaged applications
 * @param {string} installDir - Installation directory
 * @returns {string} Release directory path
 */
function getReleaseDirectory(installDir) {
  if (!installDir) {
    return null;
  }
  
  const possibleReleaseDirs = [
    path.join(installDir, 'release'),
    path.join(installDir, 'dist'),
    path.join(installDir, 'build'),
    path.join(installDir, 'out'),
  ];
...
console.log('Setting up CandyCode application...');

try {
  // Clear previous app state during installation
  ...
  // Install ESLint types for TypeScript support (if not already installed)
  console.log('\nSetting up ESLint integration...');
  try {
    execSync('npm list @types/eslint', { stdio: 'ignore' });
    console.log('✓ ESLint types already installed');
  } catch (e) {
    console.log('Installing ESLint TypeScript types...');
    execSync('npm install --save-dev @types/eslint', { stdio: 'inherit' });
    console.log('✓ ESLint types installed');
  }

  // Package the application if not already packaged
  packageAppIfNeeded();

  // Set up app icons based on the platform
  setupAppIcons();

  // Set up tiling window manager configuration and default editor associations
  setupWMConfig();

  console.log('CandyCode installation completed successfully!');
} catch (error) {
  console.error('Installation failed:', error.message);
  process.exit(1);
}

function clearPreviousAppState() {
  console.log('Clearing previous app state...');
  
  try {
    // For electron apps, settings are typically in userData directory
    // Common locations for Electron userData
    const possiblePaths = [
      path.join(process.env.HOME || process.env.USERPROFILE || '', '.config', 'CandyCode'), // Linux
      path.join(process.env.HOME || process.env.USERPROFILE || '', 'Library', 'Application Support', 'CandyCode'), // macOS
      path.join(process.env.APPDATA || process.env.HOME || process.env.USERPROFILE || '', 'CandyCode'), // Windows
    ];
    
    for (const userDataPath of possiblePaths) {
      if (fs.existsSync(userDataPath)) {
        // Remove specific files that contain user settings but preserve others
        const filesToRemove = [
          'candycode-storage', // This contains user settings, model selections, etc.
          'last-opened-project.json', // This contains the last opened project
          'ollama-active-model.json', // This contains the last selected Ollama model
        ];
        ...
        // Don't remove the entire directory as it might contain other important data
        break; // Stop after finding and clearing the first existing path
      }
    }
  } catch (error) {
    console.warn('Could not clear previous app state:', error.message);
  }
}

function packageAppIfNeeded() {
  console.log('Checking if app needs packaging...');

  // Check if there's already a packaged version
  const releaseDir = path.join(__dirname, '..', 'release');
  if (!fs.existsSync(releaseDir)) {
    fs.mkdirSync(releaseDir, { recursive: true });
  }

  let packagedExists = false;
  switch (platform) {
    case 'linux':
      const linuxPackages = glob.sync(path.join(releaseDir, 'CandyCode-*.AppImage'));
      if (linuxPackages.length > 0) {
        packagedExists = true;
      }
      break;
    case 'darwin': // macOS
      const macPackages = glob.sync(path.join(releaseDir, '*.dmg'));
      if (macPackages.length > 0) {
        packagedExists = true;
      }
      break;
    case 'win32': // Windows
      const winPackages = glob.sync(path.join(releaseDir, '*.exe'));
      if (winPackages.length > 0) {
        packagedExists = true;
      }
      break;
  }
  ...
  if (!packagedExists) {
    console.log('No packaged version found. Building and packaging the app...');
    try {
      // Build the app first
      ...
      switch (platform) {
        ...
        case 'win32': // Windows
          ...
          break;
        default:
          console.log(`Packaging not supported for platform ${platform}. Using fallback method.`);
          return;
      }
      console.log('App packaged successfully!');
    } catch (error) {
      console.warn('Could not package the app:', error.message);
      console.log('Will use development version as fallback.');
    }
  } else {
    console.log('Packaged version already exists.');
  }
}

function setupWMConfig() {
  if (platform !== 'linux') {
    console.log('WM configuration only available on Linux');
    return;
  }
  
  console.log('Setting up tiling window manager configuration...');
  
  try {
    // Run the WM config setup script
    const wmConfigScript = path.join(__dirname, 'setup-wm-config.js');
    if (fs.existsSync(wmConfigScript)) {
      execSync(`node "${wmConfigScript}" --auto`, { stdio: 'ignore' });
      console.log('✓ WM configuration and default editor associations setup');
    }
  } catch (error) {
    console.log('  WM config setup skipped (run manually with: node scripts/setup-wm-config.js --auto)');
  }
}
...
function setupAppIcons() {
  ...
  // Ensure the icon is available in the public directory
  ensurePublicIcon();
  ...
  switch (platform) {
    ...
    case 'win32': // Windows
      ...
      break;
    default:
      console.log(`Platform ${platform} not supported for icon setup`);
  }
...
function setupLinuxCommand() {
  console.log('Setting up CandyCode command...');
  ...
  try {
    ...
    // Create the CandyCode command script using dynamic installation directory
...
# Ensure we're not already running as CandyCode (prevent recursion)
if [ -n "$CANDYCODE_RUNNING" ]; then
    exec "$INSTALL_DIR" "$@"
fi
...
export CANDYCODE_RUNNING=1
export EDITOR="${INSTALL_DIR}/.local/bin/CandyCode"
...
`;
    ...
    console.log('CandyCode command created at ~/.local/bin/CandyCode');
  } catch (error) {
    console.warn('Could not set up CandyCode command:', error.message);
  }
}

function setupMacOSCommand() {
  console.log('Setting up CandyCode command for macOS...');
...
`;
    ...
    console.log('CandyCode command created at ~/bin/CandyCode');
  } catch (error) {
    console.warn('Could not set up CandyCode command for macOS:', error.message);
  }
}

function setupWindowsCommand() {
  console.log('Setting up CandyCode command for Windows...');
...
`;
    ...
    console.log('CandyCode command created at project directory as CandyCode.bat');
...
function ensurePublicIcon() {
  try {
    ...
    // Check if public/icon.png exists, if not, copy from assets
    if (!fs.existsSync(publicIconPath) && fs.existsSync(assetsIconPath)) {
      fs.copyFileSync(assetsIconPath, publicIconPath);
      console.log('Icon copied to public directory');
    }
  } catch (error) {
    console.warn('Could not ensure public icon exists:', error.message);
  }
}

// Function to resize icons to proper system sizes using ImageMagick
function resizeIcons() {
  try {
    ...
    console.log('Resizing icons to standard system sizes...');
    
    for (const size of sizes) {
      const targetDir = path.join(process.env.HOME, '.local', 'share', 'icons', 'hicolor', `${size}x${size}`, 'apps');
      if (!fs.existsSync(targetDir)) {
        fs.mkdirSync(targetDir, { recursive: true });
      }
      
      const targetPath = path.join(targetDir, `candycode.png`);
      execSync(`magick "${sourceIcon}" -resize ${size}x${size} "${targetPath}"`, { stdio: 'pipe' });
      console.log(`Created ${size}x${size} icon: ${targetPath}`);
    }
    
    // Also create the 256x256 version in the main location
    const mainTargetDir = path.join(process.env.HOME, '.local', 'share', 'icons', 'hicolor', '256x256', 'apps');
    if (!fs.existsSync(mainTargetDir)) {
      fs.mkdirSync(mainTargetDir, { recursive: true });
    }
    const mainTargetPath = path.join(mainTargetDir, 'candycode.png');
    execSync(`magick "${sourceIcon}" -resize 256x256 "${mainTargetPath}"`, { stdio: 'pipe' });
    
    return true;
  } catch (error) {
    // ImageMagick not available, suggest installation
    console.log('ImageMagick not available. Suggesting installation...');
    
    try {
      if (platform === 'linux') {
        console.log('To install ImageMagick on Linux:');
        console.log('  Ubuntu/Debian: sudo apt-get install imagemagick');
        console.log('  Fedora/RHEL: sudo dnf install ImageMagick');
        console.log('  Arch: sudo pacman -S imagemagick');
        console.log('  CachyOS: sudo pacman -S imagemagick');
      } else if (platform === 'darwin') { // macOS
        console.log('To install ImageMagick on macOS:');
        console.log('  Using Homebrew: brew install imagemagick');
      } else if (platform === 'win32') { // Windows
        console.log('To install ImageMagick on Windows:');
        console.log('  Using Chocolatey: choco install imagemagick');
        console.log('  Using Scoop: scoop install imagemagick');
        console.log('  Or download from: https://imagemagick.org/script/download.php');
      }
    } catch (e) {
      // Ignore errors in suggesting installation
    }
    
    console.log('Using original icon as-is');
    return false;
  }
}

function setupLinuxIcons() {
  try {
    // Resize icons to proper system sizes if ImageMagick is available
    const iconsResized = resizeIcons();
    
    // Copy icon to system directories (prioritizing user directory first)
    const iconPath = iconsResized ? 
      path.join(process.env.HOME, '.local', 'share', 'icons', 'hicolor', '256x256', 'apps', 'candycode.png') :
      path.join(__dirname, '..', 'assets', 'icon.png');
      
    const iconDestinations = [
      path.join(process.env.HOME, '.local/share/icons/hicolor/256x256/apps/'), // User directory (no sudo needed)
      '/usr/local/share/icons/hicolor/256x256/apps/', // Local system directory (might need sudo)
      '/usr/share/icons/hicolor/256x256/apps/' // System directory (needs sudo)
    ];

    let iconCopied = false;
    for (const dest of iconDestinations) {
      try {
        const targetDir = dest;
        if (!fs.existsSync(targetDir)) {
          fs.mkdirSync(targetDir, { recursive: true });
        }
        // Only copy if the source exists and is different from the destination
        if (fs.existsSync(iconPath) && iconPath !== path.join(targetDir, 'candycode.png')) {
          execSync(`cp "${iconPath}" "${path.join(targetDir, 'candycode.png')}"`, { stdio: 'pipe' });
        }
        console.log(`Icon copied to ${path.join(targetDir, 'candycode.png')}`);
        iconCopied = true;
        break;
      } catch (error) {
        // If we can't write to this directory, try the next one
        if (dest.startsWith('/usr')) {
          console.log(`Could not copy icon to ${dest} (permission denied), trying next location...`);
        }
      }
    }

    if (!iconCopied) {
      console.warn('Could not copy icon to any system directory');
    }

    // Create desktop entry for packaged app if it exists, otherwise for dev
    let execPath = '';
    ...
    if (releaseApps.length > 0) {
      // Use packaged AppImage
      execPath = `"${releaseApps[0]}"`;
    } else {
      // Fallback to development mode with electron
      const electronPath = execSync('which electron || echo "electron"', { encoding: 'utf8' }).trim();
      if (electronPath && electronPath !== 'electron') {
        execPath = `"${electronPath}" "${path.join(__dirname, '..')}"`;
      } else {
        // Try to use npx electron
        execPath = `npx --prefix "${path.join(__dirname, '..')}\" electron "${path.join(__dirname, '..')}"`;
      }
    }
...
`;
    ...
    console.log('Linux app icon and desktop entry created.');
  } catch (error) {
    console.warn('Could not set up Linux icons:', error.message);
  }
}
...
function convertToICNS() {
  try {
    ...
    // Create ICNS using iconutil (macOS built-in) or ImageMagick
    try {
      // Try ImageMagick first
      execSync(`magick "${sourceIcon}" -define icon:auto-resize=16,32,48,128,256,512,1024 "${icnsPath}"`, { stdio: 'pipe' });
      console.log('Created icon.icns using ImageMagick');
      return icnsPath;
    } catch {
      // If ImageMagick fails, try using sips (macOS built-in)
      try {
        execSync(`sips -s format icns "${sourceIcon}" --out "${icnsPath}"`, { stdio: 'pipe' });
        console.log('Created icon.icns using sips');
        return icnsPath;
      } catch {
        console.log('Could not create ICNS icon, using PNG as fallback');
        return sourceIcon;
      }
    }
  } catch (error) {
...
function setupMacOSIcons() {
  try {
    // On macOS, the icon is typically handled by the app bundle during packaging
    // But we can ensure the ICNS file exists for development
    convertToICNS();
    console.log('macOS icon setup - icons will be handled during app packaging.');
  } catch (error) {
    console.warn('Could not set up macOS icons:', error.message);
  }
}
...
function convertToICO() {
  try {
    ...
    try {
      execSync(`magick "${sourceIcon}" -define icon:auto-resize=16,24,32,48,64,128,256 "${icoPath}"`, { stdio: 'pipe' });
      console.log('Created icon.ico using ImageMagick');
      return icoPath;
    } catch {
      console.log('Could not create ICO icon, using PNG as fallback');
      return sourceIcon;
    }
  } catch (error) {
    console.log('Could not create ICO icon, using PNG as fallback');
...
function setupWindowsIcons() {
  try {
    // On Windows, the icon is typically handled by the installer during packaging
    // But we can ensure the ICO file exists for development
    convertToICO();
    console.log('Windows icon setup - icons will be handled during app packaging.');
  } catch (error) {
    console.warn('Could not set up Windows icons:', error.message);
  }
}

// =============================================================================
// System Tray Icon Setup
// Prepares icons for cross-platform system tray functionality
// =============================================================================
